...
const Index = () => {
  ...
  // 지식 카드 뒤집기 게임 렌더링
  if (showMemoryMatch) {
    return <MemoryMatchGame onBackToMenu={() => setShowMemoryMatch(false)} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-2 sm:p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-8 gap-4">
          <div className="text-center flex-1">
            <h1 className="text-2xl sm:text-4xl font-bold text-gray-800 mb-2 flex items-center justify-center gap-2 sm:gap-3">
              ...
              />
              <span className="hidden sm:inline">지식 플레이스 - 교육 게임</span>
              <span className="sm:hidden">지식 플레이스</span>
            </h1>
            <p className="text-sm sm:text-lg text-gray-600 px-2">재미있는 게임으로 학습하는 스마트 교육 플랫폼</p>
          </div>
          
          {/* 사용자 정보 및 로그인/로그아웃 - 모바일 최적화 */}
          <div className="flex items-center gap-2 sm:gap-4">
            {user ? (
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="text-right">
                  <p className="text-xs sm:text-sm font-medium text-gray-800">{user.nickname}님</p>
                  <p className="text-xs text-gray-500 hidden sm:block">환영합니다!</p>
                </div>
                <Button variant="outline" size="sm" className="text-xs sm:text-sm px-2 sm:px-4" onClick={logout}>
                  로그아웃
                </Button>
              </div>
            ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-8">
          <Card className="bg-white/80 backdrop-blur-sm">
            <CardContent className="p-3 sm:p-4 text-center">
              <Trophy className="mx-auto mb-1 sm:mb-2 text-yellow-500" size={20} />
              <div className="text-lg sm:text-2xl font-bold text-gray-800">{gameStats.totalGamesPlayed}</div>
              <div className="text-xs sm:text-sm text-gray-600">총 게임 수</div>
            </CardContent>
          </Card>
          <Card className="bg-white/80 backdrop-blur-sm">
            <CardContent className="p-3 sm:p-4 text-center">
              <Star className="mx-auto mb-1 sm:mb-2 text-green-500" size={20} />
              <div className="text-lg sm:text-2xl font-bold text-gray-800">{gameStats.totalCorrectAnswers}</div>
              <div className="text-xs sm:text-sm text-gray-600">정답 수</div>
            </CardContent>
          </Card>
          <Card className="bg-white/80 backdrop-blur-sm">
            <CardContent className="p-3 sm:p-4 text-center">
              <Target className="mx-auto mb-1 sm:mb-2 text-red-500" size={20} />
              <div className="text-lg sm:text-2xl font-bold text-gray-800">{gameStats.bestStreak}</div>
              <div className="text-xs sm:text-sm text-gray-600">최고 연속</div>
            </CardContent>
          </Card>
          <Card className="bg-white/80 backdrop-blur-sm">
            <CardContent className="p-3 sm:p-4 text-center">
              <BookOpen className="mx-auto mb-1 sm:mb-2 text-blue-500" size={20} />
              <div className="text-sm sm:text-lg font-bold text-gray-800">{gameStats.favoriteSubject || '없음'}</div>
              <div className="text-xs sm:text-sm text-gray-600">선호 과목</div>
            </CardContent>
          </Card>
        </div>
        ...
        {selectedSubject && selectedGameMode ? (
          ...
          <div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
              ...
              <h2 className="text-lg sm:text-2xl font-bold text-gray-800 flex items-center gap-2">
                <span className="text-xl sm:text-3xl">{selectedSubject.icon}</span>
                <span className="break-words">{selectedGrade?.name} {selectedSubject.name} - 게임 모드 선택</span>
              </h2>
            </div>
              ...
              {gameModes.map((mode) => (
                ...
                >
                  <CardHeader className="text-center p-3 sm:p-6">
                    <div className="text-2xl sm:text-4xl mb-1 sm:mb-2">{mode.icon}</div>
                    <CardTitle className="text-sm sm:text-lg">{mode.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 sm:p-6 pt-0">
                    <CardDescription className="text-center text-xs sm:text-sm mb-2 sm:mb-4">
                      {mode.description}
                    </CardDescription>
          ...
          <div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
              ...
              <h2 className="text-lg sm:text-2xl font-bold text-gray-800 flex items-center gap-2">
                <span className="text-xl sm:text-3xl">{selectedGrade.icon}</span>
                <span className="break-words">{selectedGrade.name} - 교과목 선택</span>
              </h2>
            </div>
            ...
            <div className="mb-4 sm:mb-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 max-w-4xl mx-auto">
                {/* 치즈 헌터 게임 카드 */}
                <Card 
                  className="hover:shadow-lg transition-all duration-300 cursor-pointer bg-gradient-to-r from-yellow-400 via-orange-400 to-red-400 text-white hover:scale-105"
                  onClick={() => setShowCheeseHunter(true)}
                >
                  <CardHeader className="text-center p-3 sm:p-6">
                    <div className="text-3xl sm:text-5xl mb-2 sm:mb-3">🧀</div>
                    <CardTitle className="text-lg sm:text-2xl text-white">치즈 헌터</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 sm:p-6 pt-0">
                    <CardDescription className="text-center text-white/90 text-sm sm:text-base">
                      뱀 게임과 한국어 퀴즈를 결합한 재미있는 게임!
                      <br className="hidden sm:block" />
                      <span className="flex items-center justify-center gap-2 mt-1 sm:mt-2">
                        <Gamepad2 size={14} />
                        지금 도전해보세요!
                      </span>
                    </CardDescription>
                ...
                {/* 지식 카드 뒤집기 게임 카드 */}
                <Card 
                  className="hover:shadow-lg transition-all duration-300 cursor-pointer bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 text-white hover:scale-105"
                  onClick={() => setShowMemoryMatch(true)}
                >
                  <CardHeader className="text-center p-3 sm:p-6">
                    <div className="text-3xl sm:text-5xl mb-2 sm:mb-3">🧠</div>
                    <CardTitle className="text-lg sm:text-2xl text-white">지식 카드 뒤집기</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 sm:p-6 pt-0">
                    <CardDescription className="text-center text-white/90 text-sm sm:text-base">
                      개념과 정의를 매칭하는 기억력 게임!
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
              {getSubjectsByGrade(selectedGrade.id).map((subject) => {
                ...
                return (
                  ...
                  >
                    <CardHeader className="text-center p-3 sm:p-6">
                      <div className="text-3xl sm:text-5xl mb-2 sm:mb-3">{subject.icon}</div>
          ...
          // 1단계: 학급 선택 화면
          <div>
            <h2 className="text-xl sm:text-3xl font-bold text-gray-800 mb-4 sm:mb-8 text-center flex items-center justify-center gap-2 sm:gap-3 px-4">
                ...
                return (
                  ...
                  >
                    <CardHeader className="text-center pb-3 sm:pb-4 p-4 sm:p-6">
                      <div className="text-4xl sm:text-6xl mb-2 sm:mb-4">{grade.icon}</div>
                      <CardTitle className="text-lg sm:text-2xl text-gray-800">{grade.name}</CardTitle>
                      <CardDescription className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2 px-2">
                        {grade.description}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="pt-0 p-4 sm:p-6">
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-xs sm:text-sm text-gray-600">교과목</span>
                          <Badge variant="secondary" className="text-xs">{availableSubjects.length}개</Badge>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-xs sm:text-sm text-gray-600">총 문제</span>
                          <Badge variant="outline" className="text-xs">{totalQuestions}개</Badge>
                        </div>
                        <div className="pt-1 sm:pt-2">
                          <Button className={`w-full ${grade.color} hover:opacity-90 text-white text-sm sm:text-base py-2 sm:py-3`}>
                            {grade.name} 시작하기
                          </Button>
                        </div>
                      </div>
        ...
        {/* 푸터 - 모바일 최적화 */}
        <div className="text-center mt-8 sm:mt-12 text-gray-500 px-4">
          <p className="mb-3 sm:mb-4 text-sm sm:text-base">🎓 즐겁게 학습하며 지식을 쌓아가세요! 🎓</p>
